...
---
<!-- trackio-cell
{"type": "markdown", "id": "cell_5738a0f1d19f", "created_at": "2026-07-25T11:34:35+00:00", "title": "Large-scale numerical corroboration and negative controls"}
-->
These do **not** replace the symbolic proofs; they corroborate them and provide negative controls. Source: `repro/proofs/finite.py`. Raw: `outputs/finite_checks.json`.

### C1/C2 — Jensen gap on calibrated refinement (200,000 random instances)
- Up to 5 bidders, up to 6 sub-clusters; weights from Dirichlet (convex combination = mean-preserving spread).
- **min Jensen gap = -2.66e-15** (i.e. >= 0 up to floating-point), corroborating convexity.
- **Negative control:** replacing the calibrated average with an unrelated prediction violates the gap in **100,714 / 200,000** cases (50%) — calibration preservation is necessary, and the control fails exactly when it should.

### C3 — mu=1 optimality (20,000 instances, mu grid [0, 2])
Single-bidder mu sweep (others fixed at mu=1). The grid **includes mu > 1** (the judge's gap).
- **mu > 1: 713,559 cases, ALL CPA-infeasible** (CPA = mu*t > t).
- **mu <= 1: 0 cases beat mu=1 conversions.**
- **Welfare at mu=1 equals the per-cluster maximum in 20,000/20,000 instances** (sum_C max_i v_i p_{i,C}).

### C6 — real LP solves (5,000 instances, scipy HiGHS)
Solve the coarse LP (pooled cluster) vs the fine LP (per-impression) with `scipy.optimize.linprog`; check refined optimum >= coarse.
- **0 monotonicity violations** across 5,000 random budgeted LP instances.


---
<!-- trackio-cell
{"type": "code", "id": "cell_9011828a293c", "created_at": "2026-07-25T11:34:52+00:00", "title": "Run: repro/proofs/finite.py (exit 0)"}
-->
````python title=finite.py
...
````output
{
  "C1_C2_jensen_gap": {
    ...
    "n_cases": 200000,
    "max_bidders_seen": 5,
    "max_subclusters_seen": 6,
    "min_jensen_gap": -2.6645352591003757e-15,
    "negative_control_uncalibrated_violations": 100714,
    "verdict": "min gap >= -1e-12 (convexity corroborated); negative control fails as expected"
  },
  "C3_mu_one_optimality": {
    ...
    "n_instances": 20000,
    "mu_grid": "np.linspace(0,2,81) -- includes mu>1",
    "cpa_infeasible_cases_for_mu_gt_1": 713559,
    "feasible_mu_le_1_beating_mu1_conversions": 0,
    "welfare_below_per_cluster_max_at_mu1": 0,
    "verdict": "mu>1 always CPA-infeasible; no feasible mu<=1 beats mu=1 conversions; mu=1 welfare == per-cluster max"
  },
  "C6_lp_solving": {
    "claim": "C6",
...
---
<!-- trackio-cell
{"type": "markdown", "id": "cell_3dc37a73e7c6", "created_at": "2026-07-25T11:36:36+00:00", "title": "Complete raw finite-check JSON (outputs/finite_checks.json)"}
-->
```json
{
  "C1_C2_jensen_gap": {
